...
# 2. 0번 위치에 쌓이는 자전거를 2번 위치로 옮기기.

import truck_action
import apis

x_auth_Token = '4e83586293b3e84e360a8e3adc39af40'
current_auth_key = ''
...
for rep in range(70):
    ...
    if lack_loc:
        ...
        data = truck_action.data_no_action()
        for i in range(9 - cnt_min):
            truck_action.onetime_simulate_api(base_url, auth_key, data)
        # 만약 0번 위치 자전거 5개 이상이면, 2번 위치에 2개 두고 오기.(0번위치에 자전거가 쌓이는 것 같아서.
        loc_status_ = apis.location_api(base_url, auth_key)
        if loc_status[0]['located_bikes_count'] >= 5:
            data = data = {
                "commands": [{"truck_id": 0, "command": [5, 5, 1, 1, 6, 6, 3, 3, 0, 0]},]
            }
            truck_action.onetime_simulate_api(base_url, auth_key, data)
        else:
            data = truck_action.data_no_action()
